...
public class SuccessProcessorTest {
	@InjectMocks
	...
	public void whenProcessCalledThenSaveTransactionInFailedStatus() {
		...
		Mockito.when(responseBuilder.buildResponse(request,
				transactionId,
				Status.SUCCESS)).thenReturn(response);

		TransactionResponse actualResponse = successProcessor.process(request, requestDetailsDO);
		assertEquals("response should match", response, actualResponse);
		Mockito.verify(requestService, times(1)).updateStatus(requestDetailsDO, Status.SUCCESS);
	}
}
